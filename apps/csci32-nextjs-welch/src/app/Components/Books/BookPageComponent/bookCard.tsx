...
import Comments from './bookCommentsCard'
...
export default function BookCard({ author_name, book_properties }: AuthorCardProps) {
  ...
  const hideComments = (index: number) => {
  ...
  return (
    <div className="justify-between p-2 gap-2 flex-wrap rounded-lg">
      <div className="rounded-lg">
        <ul>
          {book_properties?.map(({ book, book_synopsis, book_rating, genre, book_reccomendation, book_id }, index) => (
            <li key={index}>
              <h2 className="cursor-pointer text-purple-700 hover:underline" onClick={() => hideDetails(index)}>
              ...
              <h2 className="cursor-pointer text-purple-700 hover:underline" onClick={() => hideComments(index)}>
                {' '}
                Comments:{' '}
              </h2>
              {commentsIndex === index && (
