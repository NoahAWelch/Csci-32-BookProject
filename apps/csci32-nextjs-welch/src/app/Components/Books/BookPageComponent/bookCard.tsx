...
export default function BookCard({ author_name, book_properties }: AuthorCardProps) {
  ...
  const toggleDetails = (index: number) => {
    setDetailsIndex((prev) => (prev === index ? null : index));
  };
  ...
  return (
    <div className="justify-between p-2 gap-2 flex-wrap rounded-lg">
      <div className="rounded-lg">
        <ul>
          {book_properties?.map(
            ({ book, book_synopsis, book_rating, genre, book_reccomendation }, index) => (
              <li key={index} className="mb-4">
                ...
                <h2
                  className="cursor-pointer text-purple-700 hover:underline mt-4"
                  onClick={() => toggleComments(index)}
                >
                  Comments
                </h2>
